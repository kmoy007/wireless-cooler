...
void RFM69_ATC::sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK, bool sendRSSI, int16_t lastRSSI) {
  ...
  if (sendACK) {                   // TomWS1: adding logic to return ACK_RSSI if requested
    SPI1.transfer(RFM69_CTL_SENDACK | (sendRSSI?RFM69_CTL_RESERVE1:0));  // TomWS1  TODO: Replace with EXT1
    if (sendRSSI) {
      SPI1.transfer(abs(lastRSSI)); //RSSI dBm is negative expected between [-100 .. -20], convert to positive and pass along as single extra header byte
    ...
    SPI1.transfer(_targetRSSI ? RFM69_CTL_REQACK | RFM69_CTL_RESERVE1 : RFM69_CTL_REQACK);
  }
  else SPI1.transfer(0x00);

  for (uint8_t i = 0; i < bufferSize; i++)
    SPI1.transfer(((uint8_t*) buffer)[i]);
  unselect();
...
void RFM69_ATC::interruptHook(uint8_t CTLbyte) {
  ...
  if (ACK_RECEIVED && ACK_RSSI_REQUESTED) {
    ...
    if (DATALEN >= 1) {
      _ackRSSI = -1 * SPI1.transfer(0); //rssi was sent as single byte positive value, get the real value by * -1
